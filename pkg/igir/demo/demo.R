...
dynet.nem.region		<-	list(w=dynet.w/2,h=dynet.h/2,x=3*dynet.w/4,y=3*dynet.h/4)
##set graph layout
rdynet.graph$Layout(label=node.label, constraint=dynet.nem.region, layout="random")				
...
browser()
##change layout
rdynet.graph$Layout(label=node.label, constraint=dynet.nem.region, layout="circle")	
rdynet$GenXML()
rdynet$InvokeDyNet()